...
By default the response comes back as chat text. If you chose the "Downloadable Word document" output format, Claude instead builds a proper .docx with real headings and a real Word table for the ranking, instead of plain text.
...
- The downloadable Word document output requires the Code execution and file creation setting in Claude (Settings → Capabilities) and only works in Claude itself — pasting the prompt into another AI tool falls back to a chat response regardless of this choice.
